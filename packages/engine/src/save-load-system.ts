...
export class SaveLoadSystem {
  save(gameState: GameState, inventorySystem: InventorySystem): SaveData {
    const saveData: SaveData = {
      ...
      conversationHistory: this.mapToObject(gameState.getAllConversationHistory()),
  ...
  load(saveData: SaveData, gameState: GameState, inventorySystem: InventorySystem): void {
    gameState.loadFromSaveData(saveData);
    inventorySystem.loadFromSaveData(saveData);
  }

  async saveToFile(gameState: GameState, inventorySystem: InventorySystem, filePath: string): Promise<void> {
  ...
  async loadFromFile(filePath: string, gameState: GameState, inventorySystem: InventorySystem): Promise<void> {
  ...
  async getLatestSaveFile(): Promise<string | null> {
    const saveDir = path.join(homedir(), '.kycell', 'saves');
  ...
  async loadLatestSave(gameState: GameState, inventorySystem: InventorySystem): Promise<boolean> {
